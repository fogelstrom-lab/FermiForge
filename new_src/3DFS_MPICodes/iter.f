c-----------------------------------------------------------------------
c----                                                             ------
c----           Here comes a bunch of shuffeling routines         ------
c----           to interface the Eschrig procedure                ------
c----                                                             ------
c----                                                             ------
c-----------------------------------------------------------------------
c
c
c    Example for using the iterat subroutine: 
c    (iteration of the self consistency problem for 
c    impurity self energies at ieps'th energy)
c
c   1.) initialize common block /self/ which contains self energies
c 
c   2.) call runiterat(accuracy,numiter)
c
c-----------------------------------------------------------------------
c
      subroutine runciterat(accuracy,pmax,mmax,numiter)
c
c-----------------------------------------------------------------------
c----                                                             ------
c----        -stores impurity self energies into long array,      ------
c----        -runs iterat iteration,                             ------
c----        -stores impurity self energies back into common      ------
c----         block /self/                                     ------
c----                                                             ------
c-----------------------------------------------------------------------
c----                                                             ------
c----        INPUT:                                               ------
c----        accuracy: required accuracy                          ------
c----                  kind of accuracy see subroutine            ------
c----                  deps1 in iterat                           ------
c----        numiter: maximal allowed number of iterations       ------
c----        common block /self/ has the start self energies   ------
c----                                                             ------
c----        OUTPUT:                                              ------
c----        common block /self/ has the self consistent       ------
c----        solution for the self energies                       ------
c----                                                             ------
c-----------------------------------------------------------------------
c
...
c-----------------------------------------------------------------------
c
c      !!!! I M P O R T A N T  S T A T E M E N T !!!!
...
c-----------------------------------------------------------------------
c
c     The following parameters worked well in my problems:
c
      iruntyp=0
      prog=0.1
      kmax=numiter
...
c-------------------------------------------------------------
c
      if(myid.eq.0) call makearray(clrarray,ndeltat)
      call runciterat_bcast(ndeltat)
c
c-------------------------------------------------------------
c
      if(ndeltat.gt.NN0) then
         write(*,*) 'subroutine runiterat:'
         write(*,*) 'ndeltat too large.'
         return
      endif
c
c-------------------------------------------------------------
c
      call citerat(dfunc1,clrarray,ndeltat,print,
     .             accuracy,prog,pmax,mmax,kmax)
c
c     ndeltat was determined in makearray
c     dfunc1 has to be declared external (see above)
c     clrarray is the long vector
c
c-------------------------------------------------------------
c
      call makeself(clrarray)
c
c-----------------------------------------------------------------------
c
      return
...
c-----------------------------------------------------------------------
c
      subroutine  makearray(clrarray,npoints)
c
c-----------------------------------------------------------------------
c
      implicit none
      include 'iter.dat'      ! includes parameter statements
      integer ix,iy,icount,npoints
      real clrarray(NN0)
      ...
      common /vortex/dxx,dxy,dxz,dyx,dyy,dyz,dzx,dzy,dzz,vx,vy,vz,aa0
c
c-----------------------------------------------------------------------
c
      icount=0
      do ix=0,nx,1
         icount=icount+1
         clrarray(icount)= real(dxx(ix))
         icount=icount+1
         clrarray(icount)=aimag(dxx(ix))
         icount=icount+1
         clrarray(icount)= real(dxy(ix))
         icount=icount+1
         clrarray(icount)=aimag(dxy(ix))
         icount=icount+1
         clrarray(icount)= real(dxz(ix))
         icount=icount+1
         clrarray(icount)=aimag(dxz(ix))

         icount=icount+1
         clrarray(icount)= real(dyx(ix))
         icount=icount+1
         clrarray(icount)=aimag(dyx(ix))
         icount=icount+1
         clrarray(icount)= real(dyy(ix))
         icount=icount+1
         clrarray(icount)=aimag(dyy(ix))
         icount=icount+1
         clrarray(icount)= real(dyz(ix))
         icount=icount+1
         clrarray(icount)=aimag(dyz(ix))

         icount=icount+1
         clrarray(icount)= real(dzx(ix))
         icount=icount+1
         clrarray(icount)=aimag(dzx(ix))
         icount=icount+1
         clrarray(icount)= real(dzy(ix))
         icount=icount+1
         clrarray(icount)=aimag(dzy(ix))
         icount=icount+1
         clrarray(icount)= real(dzz(ix))
         icount=icount+1
         clrarray(icount)=aimag(dzz(ix))

         if(aa0.gt.1.0e-8) then
            icount=icount+1
            clrarray(icount)= real(vx(ix))*10
            icount=icount+1
            clrarray(icount)= real(vy(ix))*10
            icount=icount+1
            clrarray(icount)= real(vz(ix))*10
         endif
      enddo
      npoints=icount
c
...
c
      ...
      do ix=0,nx,1
         icount=icount+2
         dxx(ix)=cmplx(clrarray(icount-1),clrarray(icount))
         icount=icount+2
         dxy(ix)=cmplx(clrarray(icount-1),clrarray(icount))
         icount=icount+2
         dxz(ix)=cmplx(clrarray(icount-1),clrarray(icount))

         icount=icount+2
         dyx(ix)=cmplx(clrarray(icount-1),clrarray(icount))
         icount=icount+2
         dyy(ix)=cmplx(clrarray(icount-1),clrarray(icount))
         icount=icount+2
         dyz(ix)=cmplx(clrarray(icount-1),clrarray(icount))

         icount=icount+2
         dzx(ix)=cmplx(clrarray(icount-1),clrarray(icount))
         icount=icount+2
         dzy(ix)=cmplx(clrarray(icount-1),clrarray(icount))
         icount=icount+2
         dzz(ix)=cmplx(clrarray(icount-1),clrarray(icount))

         if(aa0.gt.1.0e-8) then
            icount=icount+1
            vx(ix)=cmplx(clrarray(icount),0.0)/10.0
            icount=icount+1
            vy(ix)=cmplx(clrarray(icount),0.0)/10.0
            icount=icount+1
            vz(ix)=cmplx(clrarray(icount),0.0)/10.0
         endif
      enddo
c
c-----------------------------------------------------------------------
