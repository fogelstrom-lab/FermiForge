c--------------------------------------------------------------------
c
c    Juhanis routine to compute the trajectory coordinates and momentum
c    directions in a cylinder if radius R starting from the point 
c    (xo,yo,zo) and with the initial direction (px,py,pz)
c
c--------------------------------------------------------------------
      subroutine trajectories(xo,yo,zo,px,py,pz,r,rsq,step,nsn,
...
C     common/reff/fxo, fx(1000),fyo,fy(1000),fzo,fz(1000) forward
C     and common/rerf/rxo,rx(1000),ryo,ry(1000),rzo,rz(1000) backward
C     The direction of the trajectory at these points will be in
C     common/fpp/fpxo,fpx(1000),fpyo,fpy(1000),fpzo,fpz(1000) forward
C     and common/rpp/rpxo,rpx(1000),rpyo,rpy(1000),rpzo,rpz(1000)
...
C     common/cfcc/nfc,fcx(1000),fcy(1000),fcz(1000) and
C     common/crcc/nrc,rcx(1000),fcy(1000),rcz(1000) where
...
C     along each half trajectory.
      ...
      real r,rsq,step
      real x1,y1,z1,x2,y2,z2,slong,q21
      real sx(0:20000),sy(0:20000),sz(0:20000)
      real rx(0:20000),ry(0:20000),rz(0:20000)
      real fx(0:20000),fy(0:20000),fz(0:20000)
      real spx(0:20000),spy(0:20000),spz(0:20000)
      real fpx(0:20000),fpy(0:20000),fpz(0:20000)
      real rpx(0:20000),rpy(0:20000),rpz(0:20000)
CC      common/cfcc/nfc,fcx(1000),fcy(1000),fcz(1000)
CC      common/crcc/nrc,rcx(1000),rcy(1000),rcz(1000)
CC      common/ccp/nisec,scx(1000),scy(1000),scz(1000)
...
C           the wall
            ...
            nspl=nspo+1
            call zigzag(nsn,step,r,rsq,x1,y1,z1,x2,y2,z2,slong,q21
     +                                      ,sx,sy,sz,spx,spy,spz)
            do 222 ii=nspl,nsn
            rx(ii)=sx(ii)
            ry(ii)=sy(ii)
            rz(ii)=sz(ii)
...
c           if(nspo.eq.1) write(6,1000) ' from traj: ',
c    +                    fx(nspo-1),asit,step,ssx,ssy,ssz,
c    +                    px,py,pz
c1000 format(a,30(x,e12.4))
            fx(nspo)=fx(nspo-1)+ssx 
            fy(nspo)=fy(nspo-1)+ssy
            fz(nspo)=fz(nspo-1)+ssz
            fpx(nspo)=px
...
C     tail end of the downward trajectory have been completed 
      ...
      subroutine zigzag(nsn,step,r,rsq,x1,y1,z1,x2,y2,z2,slong,q21
     +                                      ,sx,sy,sz,spx,spy,spz)
...
C     The former are in the array sx(1000), sy(1000), sz(1000) 
C     and the latter in the array spx(1000), spy(1000),spz(1000). 
